...
TOP_N = 150
SERVICE_RADIUS_M = 1500  # population within this radius of a site is assumed served by it
...
def main():
    ...
    # Real WorldPop population (already apportioned per 500m cell -- see enrich_features.py)
    # summed within a realistic microgrid service radius gives a genuine household estimate,
    # instead of assuming a fixed fraction of nearby OSM building footprints are residential.
    print("Loading population grid for service-area lookup...")
    feat = pd.read_csv(FEATURES_PATH, usecols=["lon", "lat", "population"])
    fx, fy = to_xy(feat["lon"].values, feat["lat"].values)
    tree = cKDTree(np.column_stack([fx, fy]))

    sx, sy = to_xy(df["lon"].values, df["lat"].values)
    neighbor_idx = tree.query_ball_point(np.column_stack([sx, sy]), r=SERVICE_RADIUS_M)
    pop_values = feat["population"].values
    served_population = np.array([pop_values[idxs].sum() for idxs in neighbor_idx])

    df["served_population"] = served_population
    df["est_households"] = (df["served_population"] / PEOPLE_PER_HOUSEHOLD).round().clip(lower=1).astype(int)
    ...
    cols = [c for c in [
        "rank", "lon", "lat", "electrification_prob", "local_suitability_score",
        "region_mean_electrification_prob", "served_population", "est_households",
        "daily_demand_kwh", "solar_pv_kw", "battery_kwh", "topology", "capex_usd"
    ] if c in df.columns]
